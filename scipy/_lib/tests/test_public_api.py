...
def test_private_but_present_deprecation(module_name, correct_module):
    ...
    for attr_name in module.__all__:
        ...
        try:
            ...
            # Some private replacement modules may not actually expose the
            # attribute; make the failure informative rather than raising
            # an unexpected AttributeError that bypasses the warning check.
            pytest.fail(
                f"Expected attribute {attr_name!r} to be present on {module_name!r} "
                "(with a deprecation warning), but AttributeError was raised. "
                "Ensure the private replacement module exposes the attribute or "
